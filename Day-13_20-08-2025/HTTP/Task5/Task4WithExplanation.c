...
#include <string.h>     // For memset(), strcmp(), strstr(), sscanf()
#include <unistd.h>     // For close(), read(), write(), sleep()
#include <arpa/inet.h>  // For inet_ntoa(), htons(), htonl(), inet_addr()
...
// ============================================================================
// recv_all_headers()
// Reads all HTTP request headers from client until "\r\n\r\n" (end of headers)
...
int recv_all_headers(int client_fd, char *buffer, size_t size) {
    ...
    // Debugging: Print full HTTP request received
...
int main() {
    ...
    socklen_t addr_size;
    char buffer[4096];  // Buffer to hold incoming request
    ...
    // -------------------- Step 2: Reuse address --------------------
    int opt = 1;
    setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    // SO_REUSEADDR allows server to restart immediately after crash/close
    ...
    listen(server_fd, 5); // backlog = 5 clients
    printf("Server listening on port %d...\n", PORT);

    // -------------------- Step 5: Accept and handle clients --------------------
    while (1) {
        ...
        if (strcmp(path, "/hello") == 0) {
            ...
            char *time_str = ctime(&now); // Returns human-readable time (with newline)
            char body[256];
